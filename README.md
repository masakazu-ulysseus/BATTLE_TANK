...
- **アイテムキャリア敵**: ステージ内の決まった順番（2・6・9番目）に出現する敵が赤く点滅し、撃破でアイテムをドロップ（本家方式）
...
### 目的
各ステージの敵戦車10機をすべて破壊しながら、自分の基地を守りましょう。35ステージすべてをクリアするとゲームクリアです。
...
### アイテムキャリア敵（本家方式）
各ステージで**2番目・6番目・9番目**に出現する敵は**赤く点滅**しており、撃破するとアイテムをドロップします。
（本家の出現位置 4・11・18/20機 を敵数10機に比率換算したもの。`ENEMIES_PER_STAGE` に連動して自動算出されます）
...
#### 基本ゲーム設定
- `ENEMIES_PER_STAGE`: ステージごとの敵数 (現在: 10、バランス調整値。本家は20)
